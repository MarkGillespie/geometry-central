...
std::vector<Dart<D>> CombinatorialMap<D>::adjacentDarts(Cell<k, D> cell) {
  ...
  while (!dartsToVisit.empty()) {
    ...
    for (std::pair<Dart<D>, bool> n : orbitNeighbors<k>(curr)) {
      if (seenDarts.find(n.first) == seenDarts.end()) {
        dartsToVisit.push_back(n.first);
        seenDarts.insert(n.first);
      }
    }
    ...
    // if (k == 0) { // when k = 0, orbit over all map[i].map[j] for i < j
    //   for (size_t iMap = 1; iMap < D; ++iMap) {
    //     for (size_t jMap = 0; jMap < iMap; ++jMap) {
    //       Dart<D> iPartner = curr.partner(iMap);
...
std::vector<Cell<k2, D>> CombinatorialMap<D>::adjacentCells(Cell<k1, D> cell, bool useDirectLoop) const {
  ...
  if (useDirectLoop == false) {
    while (!dartsToVisit.empty()) {
      ...
      if (true) {
        ...
        std::cout << "debugging orbit neighbors for dart " << currDart << " | k = " << k1 << std::endl;
        for (size_t iMap = 0; iMap < D; ++iMap) {
          std::cout << "   dartMap[" << iMap << "][d] = " << currDart.partner(iMap) << std::endl;
        }
        for (size_t iMap = 0; iMap < D; ++iMap) {
          if (iMap + 1 != k1) { // iMap != k-1
            Dart<D> next = currDart.partner(iMap);
            if (next == currDart) continue; // skip (INVALID_IND)
            bool orientationPreserving = size_t(iMap + 1) < k1;
            std::cout << " explicit loop | iMap: " << iMap << ", next: " << next
                      << ", orientationPreserving: " << (orientationPreserving ? "true" : "false") << std::endl;
          }
        }
        for (auto it = orbitNeighbors<k1>(currDart).begin(); it != orbitNeighbors<k1>(currDart).end(); ++it) {
          std::cout << " iterator | iMap: " << it.iMap << ", next: " << (*it).first
                    << ", orientationPreserving: " << ((*it).second ? "true" : "false") << std::endl;
        }
      } else {
        for (size_t iMap = 1; iMap < D; ++iMap) {
          for (size_t jMap = 0; jMap < iMap; ++jMap) {
            ...
            std::cout << " explicit loop | iMap: " << iMap << ", jMap: " << jMap << ", next: " << next
                      << ", orientationPreserving: " << (orientationPreserving ? "true" : "false") << std::endl;
          }
        }
        for (auto it = orbitNeighbors<0>(currDart).begin(); it != orbitNeighbors<0>(currDart).end(); ++it) {
          std::cout << " iterator | iMap: " << it.iMap << ", jMap: " << it.jMap << ", next: " << (*it).first
                    << ", orientationPreserving: " << ((*it).second ? "true" : "false") << std::endl;
        }
      }
...
void CombinatorialMap<D>::indexCells() {
  ...
  for (Dart<3> d : darts()) {
    for (std::pair<Dart<3>, bool> n : orbitNeighbors<k>(d)) {
      ...
      if (DEBUG_PRINT) {
        std::cout << "uniting dart " << d.getIndex() << " with dart " << n.first.getIndex() << " via map " << iMap
