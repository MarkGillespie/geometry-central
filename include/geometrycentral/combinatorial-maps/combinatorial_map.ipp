...
template <size_t D>
template <size_t k>
std::vector<std::pair<Dart<D>, bool>> CombinatorialMap<D>::orbitNeighbors(Dart<D> d) const {
  static_assert(k <= D, "cell dimension k must be less than or equal to complex dimension D");
  std::vector<std::pair<Dart<D>, bool>> neighbors;

  if (k == 0) { // when k = 0, orbit over all map[i].map[j] for i < j
    neighbors.reserve(D * (D - 1) / 2);
    for (size_t iMap = 1; iMap < D; ++iMap) {
      for (size_t jMap = 0; jMap < iMap; ++jMap) {
...
void CombinatorialMap<D>::indexCells() {
  ...
  //       if (DEBUG_PRINT) {
  //         std::cout << "uniting dart " << iDart << " with dart " << jDart << " via map " << iMap
